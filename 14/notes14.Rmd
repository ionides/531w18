---
title: "14. Case study: POMP modeling to investigate financial volatility"
...
* Recall the daily S&P 500 data that we saw earlier, in [Section 3.5](http://ionides.github.io/531w18/03/notes03.html#a-random-walk-model), downloaded from yahoo.com.
...
* Look at the plots in  [Section 3.5](http://ionides.github.io/531w18/03/notes03.html#a-random-walk-model). 
...
* In  `r round(t.pf1["elapsed"],1)` seconds, we obtain a log likelihood estimate of `r round(L.pf1[1],2)` with a Monte standard error of `r round(L.pf1[2],2)`. Notice that the replications are averaged using the `logmeanexp` function, since the likelihood estimate is unbiased on the natural scale but not the log scale.
...
* Also, it might help to refine our inference be computing a likelihood profile over $\sigma_\nu$.
