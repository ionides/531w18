---
title: "2. Time series models, trend and autocovariance"
...
* A time series is a sequence of numbers, called **data**. In general, we will suppose that there are $N$ numbers, $\data{y_1},\data{y_2},\dots,\data{y_N}$, collected at an increasing sequence of times, $t_1,t_2,\dots,t_N$. 

    + We write $1{\mycolon}N$ for the sequence $\{1,2,\dots,N\}$ and we write the collection of numbers $\{\data{y_n}, n=1,\dots,N\}$ as $\data{y_{1:N}}$.
...
$$ f_{Y_{1:N}}(y_1,\dots,y_N \given \theta).$$
    ...
    + Scientifically, we postulate that $\data{y_{1:N}}$ are a realization of $Y_{1:N}$ for some unknown value of $\theta$.

* Our notation distinguishes between 

    + the model, $Y_{1:N}$
    + an arbitrary realization of the model, $y_{1:N}$
    + the specific sequence of numbers that we observed as data, $\data{y_{1:N}}$
...
* We call $\data{\hat\mu_n}$ an **estimated mean function** or **estimated trend**.
...
$$\hat\mu(y_{1:N})=\frac{1}{N}\sum_{k=1}^N y_k.$$
In this case, the corresponding estimate $\data{\hat\mu}=\hat\mu(\data{y_{1:N}})$ is the sample mean.

    + We can compute the sample mean, $\data{\hat\mu}$, for any dataset. It is only a reasonable estimator of the mean function when a mean stationary model is appropriate.
...
* For a covariance stationary model, and some mean function estimator $\hat\mu_n=\hat \mu_n(y_{1:N})$, a common estimator for $\gamma_h$ is
...
we have 
$$ \mu = Z\beta.$$
We also write a generic potential dataset $y_{1:N}$ and the data $\data{y_{1:N}}$ as column vectors,
$$\begin{eqnarray}
...
$$\data{\beta}=\hat\beta_{OLS}(\data{y_{1:N}}) = (Z^\transpose Z)^{-1}Z^\transpose \data{y}.$$
...
* Under model L1, the estimator $\hat\beta_{OLS}(y_{1:N})$ is unbiased. This can be checked:
$$\begin{eqnarray}
...
* The variance/covariance matrix of $\hat\beta_{OLS}(Y_{1:N})$ under this model is
